...
type TrackingFilter = "all" | "pending" | "approved" | "cancelled" | "dispatchIssued" | "delivered" | "invoiced";
...
export default function ManagerOrderTrackingPage() {
  ...
  const filteredOrders = useMemo(() => {
    return [...orders]
      ...
      .filter((order) => {
        const matchesSearch =
          order.code.toLowerCase().includes(search.toLowerCase()) || order.createdBy.toLowerCase().includes(search.toLowerCase());

        if (!matchesSearch) return false;
        if (filter === "all") return true;
        if (filter === "dispatchIssued") return order.warehouseStatus === "dispatchIssued";
        if (filter === "delivered") return order.warehouseStatus === "delivered";
  ...
  const columns: DataTableColumn<ExpertOrder>[] = [
    ...
    { key: "stage", header: "آخرین مرحله", render: (row) => getOrderLastStageLabel(row) },
    { key: "updated", header: "تاریخ آخرین تغییر", render: (row) => formatDate(row.updatedAt) },
    {
      key: "actions",
      header: "عملیات",
      render: (row) => (
  ...
  return (
    <DashboardLayout role="manager" title="روند سفارش ها">
      <SectionHeader title="ردیابی فرآیند سفارش" description="پایش کامل سفارش ها از مرحله تایید تا فاکتور" />
      ...
      {filteredOrders.length > 0 ? (
        ...
        <EmptyState title="سفارشی با این فیلتر یافت نشد" description="فیلترهای انتخابی را تغییر دهید." />
      )}
    </DashboardLayout>
  );
}
